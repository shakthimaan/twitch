...
On these systems, ~user-id/ is support only for the logged in current user.

To prevent Emacs from inserting the default directory when reading
file names, change the variable inser-default-directory to t.
...
expands automatically to accomodate the text.
